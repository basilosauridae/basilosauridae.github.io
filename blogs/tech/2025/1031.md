...
Temporal Dead Zone 暂时性死区，用 `let、const、class声明的变量`可以称其从代码块的开始一直到代码执行到变量声明的位置并被初始化前，都处于一个“暂时性死区”中。

## Vue3文档涉及相关

### IDE

**Integrated Development Environment,集成开发环境**，用于提供程序开发环境的应用程序，包括代码编辑器、编译器、调试器和图形用户界面等工具。通俗来说就是程序员用来写代码、调试和运行软件的一站式工具包‌。如Visual Studio。

:::info
**我平时用的vs code属于IDE吗**？我注意到Visual Studio是IDE，它和VS code有什么关系?
[download](https://visualstudio.microsoft.com/zh-hans/free-developer-offers/)
![placeholder](https://cetacea-1304984885.cos.ap-shanghai.myqcloud.com/pieces/vstools.png)

VS Code 严格意义上不算传统 IDE，而是一个“代码编辑器”；但它通过插件扩展后，体验上非常接近 IDE。

虽然都姓“Visual”，且都是微软出品，但它们是两个独立开发、架构完全不同的产品：

- 用 Visual Studio：如果你主要写 C#、.NET MAUI 或大型 C++ 桌面/游戏项目（比如 Unity 开发），请毫不犹豫选它，因为只有它自带完整的项目模板和 MSBuild 编译系统。

- 用 VS Code：如果你写 Web 前端（Vue/React）、Python、Go、Rust，或者需要远程连接 Linux 服务器开发，用 VS Code 体验极佳。

尽管代码不互通，但两者共享了品牌（“Visual”家族）和核心调试引擎。微软特意把 Visual Studio 中强大的调试器（Debugger）核心剥离出来，适配到了 VS Code 中。因此，你在 VS Code 里按 F5 调试代码时，手感其实很像在用 Visual Studio。
:::
...
今天我在写`npm create vue@latest`时，写成了`lasted`，提示我npm没有这个命令。这个错误太低级了，我得写下来。

| word | POS | meaning | Typical use case |
|-------|-------|-------|-------|
| latest | adj. | 最新的 | latest version |
| lasted | v. | 持续 | The meeting lasted 2 hours. |
