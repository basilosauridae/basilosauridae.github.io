...
```
>查阅资料后AI告诉我了一个概念：**Duck Typing**, pic from chatgpt：

![placeholder](https://cetacea-1304984885.cos.ap-shanghai.myqcloud.com/pieces/duckTyping.png)

deepseek给出的解释是[站内博文cmd+f搜鸭式类型](/blogs/tech/2026/0805.html#对文档内容的思考和延伸)

ChatGPT认为资料中将`Duck Typing`和`TypeScript中的Structural Typing`放在一起讲，理解层面没什么大问题，但不能把他们混为一谈。

如`Python中的Duck Typing`不会在编译阶段检查，在运行时检查；`TypeScript中的Structural Typing`会在编译时检查。它们背后的哲学非常接近：**关注对象具有什么能力，而不是它的名义身份。** 但实现机制不同。

为什么需要Duck Typing？**因为软件系统中的函数通常并不需要知道一个对象“完整是什么”，只需要知道它是否拥有完成当前任务所必需的能力。**

> Program to interfaces, not implementations.依赖能力，而不是依赖身份。=> 函数应该尽量声明自己真正需要的最小结构。 => **降低“耦合”*
