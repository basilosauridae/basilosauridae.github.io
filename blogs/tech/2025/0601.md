...
起因是我在做[买卖股票的最佳时机](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock/submissions/725440878/?envType=study-plan-v2&envId=selected-coding-interview)时，有个奇妙的题解`value = Infinity`，`Infinity`在js中是无穷大的意思，引起了我的注意。

如果不这样赋值，需要写成循环，这里赋值为Infinity的写法可以统一处理循环，简化代码。这种方法称为**哨兵技巧，用一个虚拟的、特殊的值或节点，简化边界条件的判断，避免写额外的if语句去处理第一次为false的情况。**

## 算法细节优化

### 二分查找中的mid优化

[Binary Search](http://leetcode.cn/problems/binary-search/description/?envType=study-plan-v2&envId=selected-coding-interview)中，对mid的赋值优化，从直观的`Math.floor((left + right) / 2)`优化为`left + Math.floor((right-left)/2)`。

在 JavaScript 中直接写 `Math.floor((left + right) / 2)` 也是完全可行的，因为它不会溢出，结果也正确。

但采用 `left + Math.floor((right - left) / 2)` 是一个跨语言的安全习惯，可以避免潜在溢出问题，并且让代码更易移植。同时，它明确地表达了**从 left 开始，加上区间长度的一半**的几何意义，在理解二分查找的缩小区间过程时也很有帮助。
...
面试中喜欢考察，是因为在实际应用中内存有限，程序设计者需要有意识**节省内存**。

[旋转图像](https://leetcode.cn/problems/rotate-image/description/?envType=study-plan-v2&envId=selected-coding-interview)的精髓在于设置一个temp变量来辅助交换：
...
[旋转字符串](https://leetcode.cn/problems/rotate-string/description/?envType=study-plan-v2&envId=selected-coding-interview)中，`s+s`正好提供了所有可能的起始位置来截取子串，所以`s+s`是最优选择。
